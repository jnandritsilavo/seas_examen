...
class elFinder {
	...
	public function exec($cmd, $args) {
		...
		$result['action'] = $cmd; // Get cmd action
	...
	protected function open($args) {
		...
		//print_d($result['files']);
		return $result;
	}


	/**
	*	Récupérer dossier by droit
	*/

	/*protected function get_files() {
		$zQuery = ' SELECT * FROM ' . TABLE_CABINET . ' ';
		$toResult = $this->CI->db->query($zQuery)->result();
		$tzFiles = array();
		foreach($toResult as $k => $zFile) {
			$tzFiles[$k][] = array(
							"isowner" => '',
		                    "mime" => 'directory',
		                    "ts" => '1454493911'.$k.',
